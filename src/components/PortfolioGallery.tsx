...
import { PortfolioSliderModal } from './PortfolioSliderModal';
...
export const PortfolioGallery: React.FC<PortfolioGalleryProps> = ({ onBookSpecial }) => {
  ...
  return (
    <section id="gallery-section" className="py-16 bg-slate-50 border-t border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-10">
          ...
          <p className="text-slate-600 text-xs sm:text-sm">
            شاهد نتائج كادرنا في تزيين الصالات والمنازل والمستشفيات وسيارات الزفاف في بغداد والمحافظات. انقر على «عرض التفاصيل» لتصفح سلايدر الصور عالي الدقة لكل عمل.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {filteredItems.map(item => {
            ...
            return (
              ...
              >
                {/* Photo & Clickable Slider Trigger */}
                <div 
                ...
                >
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-slate-950/20 to-transparent group-hover:via-slate-950/40 transition-colors" />
                  
                  {/* Photo count pill badge (Top Left) */}
                  <div className="absolute top-3 left-3 bg-slate-900/80 backdrop-blur-xs text-white text-[11px] font-bold px-2.5 py-1 rounded-lg flex items-center gap-1.5 border border-white/10 shadow-sm">
                    <Images className="w-3.5 h-3.5 text-amber-400" />
                    <span>{photoCount} لقطات HD</span>
                  </div>

                  {/* Location Badge (Top Right) */}
                  <div className="absolute top-3 right-3 bg-slate-900/80 backdrop-blur-xs text-white text-[11px] font-bold px-2.5 py-1 rounded-lg flex items-center gap-1 border border-white/10 shadow-sm">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <span className="bg-rose-600/90 text-white font-bold text-xs px-4 py-2 rounded-xl backdrop-blur-xs flex items-center gap-2 shadow-lg transform translate-y-2 group-hover:translate-y-0 transition-transform">
                      <Eye className="w-4 h-4" />
                      <span>عرض التفاصيل والسلايدر</span>
                    </span>
                  </div>
                ...
                <div className="p-5 flex-1 flex flex-col justify-between space-y-3">
                  ...
                  {/* Bottom Actions Row: (عرض التفاصيل) & (طلب تصميم مشابه) */}
                  <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        onClick={() => setSelectedItem(item)}
                        className="px-3 py-1.5 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 text-xs font-bold transition-colors flex items-center gap-1.5 cursor-pointer border border-rose-200"
                        title="عرض تفاصيل الباقة وتصفح سلايدر الصور"
                      >
                    ...
                    <button
                      ...
                      className="font-bold text-xs text-slate-700 hover:text-rose-600 cursor-pointer flex items-center gap-1 transition-colors"
                    >
      ...
      {/* Interactive Photo Slider & Details Modal */}
      <PortfolioSliderModal
        item={selectedItem}
        isOpen={Boolean(selectedItem)}
        onClose={() => setSelectedItem(null)}
        onBookSpecial={onBookSpecial}
      />
    </section>
  );
};
